...
function cargarLista() {
	...
	$.ajax({
		type: "POST",
		url: "../../../controladores/casos/listarCasosAgendados.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
		...
		success: function(data){
			...
			if (datos) {
				...
				for(i=0; i< datos.length; i++) {
					...
					dataSet.push([datos[i].id_caso, datos[i].telefono, datos[i].usrname, linkEncuesta, datos[i].estado, datos[i].agendaF, botonDetalleCaso+botonDetalleHist])
...
$(document).ready(function() {
	...
	//Activar Menu
	$('li#casosAgendados').addClass('active');
...
$('#descargarExcel').on('click', function () {
	...
	$.ajax({
		type: "POST",
		url: "../../../controladores/descargas_excel/descargaExcelCasosAgendados.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
			//$("#login").val('Conectando...');
...
$('#Eliminar').on('click', function () {
	...
	var stringDatos = ''
		...
		$.ajax({
			type: "POST",
			url: "../../../controladores/descargas_excel/elimnarExcelFilesCasosAgendados.php",
			data: stringDatos,
			cache: false,
			beforeSend: function(){
