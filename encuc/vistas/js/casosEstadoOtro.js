...
function cargarLista() {
	...
	var stringDatos = 'idEstado=13';
	$.ajax({
		type: "POST",
		url: "../../../controladores/casos/listarCasosXEstado.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
		...
		success: function(data){
			...
			if (datos) {
				...
				for(i=0; i< datos.length; i++) {
					...
					dataSet.push([datos[i].id_caso, datos[i].telefono, datos[i].usrname, linkEncuesta, datos[i].valor, datos[i].fecha, botonDetalleCaso+botonDetalleHist])
...
$(document).ready(function() {
	...
	//Activar Menu
	$('li#casosEstadoOtro').addClass('active');
...
$('#descargarExcel').on('click', function () {
	...
	$.ajax({
		type: "POST",
		url: "../../../controladores/descargas_excel/descargaExcelCasosOtro.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
			//$("#login").val('Conectando...');
...
$('#Eliminar').on('click', function () {
	...
	var stringDatos = ''
		...
		$.ajax({
			type: "POST",
			url: "../../../controladores/descargas_excel/elimnarExcelFilesCasosOtro.php",
			data: stringDatos,
			cache: false,
			beforeSend: function(){
