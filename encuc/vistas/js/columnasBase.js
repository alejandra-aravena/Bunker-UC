function cargarLista() {
	...
	$.ajax({
		type: "POST",
		url: "../../../controladores/columnas_base/listarColumnasBase.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
		...
		success: function(data){
			...
			if (datos) {
				...
				for(i=0; i< datos.length; i++) {
					...
					var row = $('<tr></tr>');
					row.append($('<td></td>').text(datos[i].Field));
					row.append($('<td></td>').text(datos[i].Type));
					
					if ((datos[i].Field != 'id_caso') && (datos[i].Field != 'telefono')) {
						...
						row.append($('<td></td>')
									.append($('<button></button>')
										...
										.attr('data-idvalue', datos[i].Field)
										.attr('data-input1value', datos[i].Field)
										.attr('data-input2value', datos[i].Type)
										.val('Editar-'+datos[i].Field)
										.append($('<span></span>')
...
$(document).ready(function() {
	...
	//Activar Menu
	$('li#columnasBase').addClass('active');
...
$('#agregarModal').on('show.bs.modal', function (event) {
  ...
  $(this).find('.modal-title').text('Agregar Columna Base');
})
...
$('#agregarBtn').on('click', function () {
	...
	if (btnVal != '') {
		...
		var stringDatos = 'columna='+ btnVal+'&tipo='+btnVal2;
		
		$.ajax({
			type: "POST",
			url: "../../../controladores/columnas_base/agregarColumnasBase.php",
			data: stringDatos,
			cache: false,
			beforeSend: function(){
				//$("#login").val('Conectando...');
...
$('#editarModal').on('show.bs.modal', function (event) {
  ...
  $(this).find('.modal-title').text('Editar Columna Base');
...
$('#editarBtn').on('click', function () {
	...
	if (btnVal != '') {
		...
		var stringDatos = 'nuevaColumna='+ btnVal+'&columna='+ idVal+'&tipo='+ btnVal2;
		
		$.ajax({
			type: "POST",
			url: "../../../controladores/columnas_base/editarColumnasBase.php",
			data: stringDatos,
			cache: false,
			beforeSend: function(){
				//$("#login").val('Conectando...');
...
$('#eliminarModal').on('show.bs.modal', function (event) {
  ...
  $(this).find('.modal-title').text('Eliminar Columna Base');
...
$('#eliminarBtn').on('click', function () {
	...
	var stringDatos = 'columna='+ idVal;
		
	$.ajax({
		type: "POST",
		url: "../../../controladores/columnas_base/eliminarColumnasBase.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
			//$("#login").val('Conectando...');
