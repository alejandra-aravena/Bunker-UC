...
function cargarLista() {
	...
	$.ajax({
		type: "POST",
		url: "../../../controladores/casos/listarCasos.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
		...
		success: function(data){
			...
			if (datos) {
				...
				for(i=0; i< datos.length; i++) {
					...
					var botonLlamadas = '<button type="button" class="btn btn-warning btn-sm" aria-label="DetalleLlamadas" data-toggle="modal" data-target="#detalleLlamadasModal" data-id_caso="'+datos[i].id_caso+'" data-telefono="'+datos[i].telefono+'" value="Llamadas-'+datos[i].id_caso+'"><span class="glyphicon glyphicon-phone-alt" aria-hidden="true"></span></button>'
					
					dataSet.push([datos[i].id_caso, datos[i].telefono, datos[i].usrname, linkEncuesta, datos[i].estado, datos[i].fecha, datos[i].intentos_llamada, datos[i].intentos_fservicio, botonDetalleCaso+botonDetalleHist+botonLlamadas])
...
//Modal detalleCasoModal
$('#detalleLlamadasModal').on('show.bs.modal', function (event) {
	var button = $(event.relatedTarget)
  	var idCaso = button.data('id_caso')
	var telefono = button.data('telefono')
	
	$('#telefonoDescarga').val(telefono)
	$('#llamadasCDR h5 strong').text(telefono)
  
})

//Boton descargar zip
$('#descargarAudio').on('click', function () {
	if ($('#ano3').val() == '') {
	...
	var mes = $('#mes3').val()
	var ano = $('#ano3').val()
	var telefono = $('#telefonoDescarga').val()
	...
	var stringDatos = 'ano='+ ano+'&mes='+ mes+'&dia='+ dia+'&telefono='+ telefono;
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#detalleLlamadasModal').modal('hide')
			}
		});
});
...
$('#descargarExcel').on('click', function () {
	...
	$.ajax({
		type: "POST",
		url: "../../../controladores/descargas_excel/descargaExcelCasos.php",
		data: stringDatos,
		cache: false,
		beforeSend: function(){
			//$("#login").val('Conectando...');
...
$('#Eliminar').on('click', function () {
	...
	var stringDatos = ''
		...
		$.ajax({
			type: "POST",
			url: "../../../controladores/descargas_excel/elimnarExcelFilesCasos.php",
			data: stringDatos,
			cache: false,
			beforeSend: function(){
